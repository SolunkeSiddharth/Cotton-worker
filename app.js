...
class CottonTracker {
    ...
    setupEventListeners() {
        ...
        // Generate Report
        document.getElementById('generate-report-btn').addEventListener('click', () => this.showReportDialog());
        ...
        this.currentSession.forEach((entry, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                    ...
                    ${!entry.saved ? `<button class="action-btn action-btn--save" onclick="cottonTracker.saveEntry(${entry.id})">Save</button>` : ''}
                </td>
            `;
        ...
        this.showConfirmDialog(
            ...
            async () => {
                ...
                try {
                    ...
                    let totalKg = 0;
                    let totalAmount = 0;
                    const entries = this.currentSession.map(entry => {
                        totalKg += entry.kg;
                        totalAmount += entry.total;
                        return {
                            name: entry.name,
                    ...
                    return new Promise((resolve, reject) => {
                        // Save to history (will overwrite if exists)
                        ...
                        historyRequest.onsuccess = () => {
                            ...
                            clearRequest.onsuccess = async () => {
                                await this.loadCurrentSession();
                                await this.loadHistory();
                                this.showToast(`Day completed for ${formattedDate}`, 'success');
                                resolve();
                            };
    ...
    // PDF Generation
    showReportDialog() {
        // For simplicity, generate a full report
