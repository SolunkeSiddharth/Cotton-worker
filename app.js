...
class CottonTracker {
    ...
    renderCurrentSession() {
        ...
        this.currentSession.forEach((entry, index) => {
            ...
            row.innerHTML = `
                <td class="marathi-text">${entry.name}</td>
                <td>${entry.kg}</td>
                <td>₹${entry.rate}</td>
            ...
            <div class="history-item">
                ...
                <div class="history-details" id="details-${record.date}">
                    <div class="history-actions">
                        <button class="btn btn--outline btn--sm" onclick="cottonTracker.generateDayPDF('${record.date}')">📄 Generate PDF</button>
                        <button class="btn btn--outline btn--sm" onclick="cottonTracker.deleteHistoryDay('${record.date}')">🗑️ Delete Day</button>
                    </div>
    ...
    async generateDayPDF(date) {
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                request.onsuccess = () => {
                    const historyRecord = request.result;
                    this.createUnicodePDF([historyRecord], `CottonReport${date.replace(/-/g, '')}.pdf`);
                    resolve();
                };
                request.onerror = () => reject(request.error);
            });
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                request.onsuccess = () => {
                    ...
                    this.createUnicodePDF(history, 'CottonFullReport.pdf');
                    resolve();
                };
                request.onerror = () => reject(request.error);
            });
    ...
    // Enhanced PDF Generation with Unicode Support
    createUnicodePDF(historyRecords, filename) {
        ...
        try {
            ...
            // Set font to support Unicode (this will use the built-in fonts)
            ...
            historyRecords.forEach((record, recordIndex) => {
                ...
                record.entries.forEach((entry, index) => {
                    // For Marathi names, we'll use transliteration or fallback to English display
                    const displayName = this.prepareNameForPDF(entry.name);
                    tableData.push([
                        `${index + 1}`,
                        displayName,
                        entry.kg.toString(),
                ...
                if (doc.autoTable) {
                    doc.autoTable({
                        head: [['S.No.', 'Worker Name', 'KG', 'Rate', 'Total']],
                        body: tableData,
                        startY: yPosition,
                        styles: {
                            fontSize: 10,
                    ...
                    // Table header
                    const headers = ['S.No.', 'Worker Name', 'KG', 'Rate', 'Total'];
                    let xPos = 20;
            ...
            if (historyRecords.length > 1) {
                ...
                if (doc.autoTable) {
                    doc.autoTable({
                        ...
                        headStyles: {
                            fillColor: [40, 167, 69] // Green for totals
                        },
                        margin: { left: 20, right: 20 }
                    });
                } else {
                    ...
                    yPosition += 10;
                    doc.text(`Total Amount: Rs.${grandTotalAmount.toFixed(2)}`, 20, yPosition);
                }
            }
    ...
    // Helper function to prepare names for PDF display
    prepareNameForPDF(name) {
        // This function can be enhanced to handle Marathi transliteration
        // For now, we'll ensure the name is properly encoded
        try {
            // If name contains Devanagari characters, we'll add a note
            if (/[\u0900-\u097F]/.test(name)) {
                // Contains Devanagari script
                return name; // Keep original - jsPDF will try its best
            }
            return name;
        } catch (error) {
            return name;
